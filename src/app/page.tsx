...
export default function Home() {
  ...
  return (
    <div className="scene-container overflow-hidden bg-background">
      <Canvas camera={{ position: [0, 0, 8], fov: 45 }}>
        {/* Adds 4 pages of scrollable space to the 3D Scene */}
        <ScrollControls pages={4} damping={0.2} maxSpeed={0.5}>
           {/* The actual 3D objects */}
           <Scene />

           {/* The HTML overlay synchronized with scroll */}
              ...
              <section ref={ctaRef} className="h-screen min-h-[calc(100vh_-_theme(space.14))] sm:min-h-[calc(100vh_-_theme(space.16))] flex items-center justify-center p-4 sm:p-6 md:p-8 relative">
                ...
                <div className="glass-mobile responsive-padding rounded-[1.5rem] sm:rounded-[2rem] max-w-xs sm:max-w-2xl md:max-w-4xl text-center space-y-6 sm:space-y-8 w-full">
                  ...
                  <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 pt-6 sm:pt-8">
                    <Link href="/auth">
                      <button className="w-full sm:w-auto touch-button bg-white text-primary border border-primary/20 rounded-full text-base sm:text-lg font-semibold hover:bg-gray-50 transition-colors cursor-pointer pointer-events-auto">
                        Sign In
                      </button>
                    </Link>
                    <Link href="/assessment">
                      <button className="w-full sm:w-auto touch-button bg-primary text-primary-foreground rounded-full text-base sm:text-lg font-semibold hover:scale-105 transition-transform shadow-md cursor-pointer pointer-events-auto">
                        Try Anonymous
                      </button>
                    </Link>
                  </div>
