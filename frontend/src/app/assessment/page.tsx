...
const assessmentQuestions = [
  ...
  },
  {
    id: 4,
    text: "ENERGY",
    question: "Have you felt tired or had little energy over the past few weeks?",
  },
  {
    id: 5,
    text: "INTEREST",
    question: "Have you experienced a loss of interest or pleasure in doing things you usually enjoy?",
  }
];
...
export default function AssessmentPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await assessmentAPI.submit({ answers: answersArray, totalScore });
      // Store the full response for dashboard
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="glass-mobile responsive-padding rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[3rem] shadow-2xl border border-white/40 backdrop-blur-3xl">
            <span className="inline-block px-3 sm:px-4 py-1.5 rounded-full bg-primary/10 text-primary text-xs font-bold tracking-widest mb-4 sm:mb-6">
              {currentQ.text}
            </span>
            ...
            <div className="flex justify-between items-center mt-8 sm:mt-10 md:mt-12 pt-4 sm:pt-6 border-t border-border/30">
              ...
              {currentStep === assessmentQuestions.length - 1 ? (
                ...
                >
                  {isSubmitting ? 'Analyzing...' : 'Complete'}
                  {!isSubmitting && <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5" />}
                </button>
              ) : (
                <button
                  onClick={() => setCurrentStep(Math.min(assessmentQuestions.length - 1, currentStep + 1))}
